...
void Recom::revise_missing_values_new(void)  // 7500
{
    int tmprow, tmpcol;
    for (int m = 0; m < Missing;) {
        ...
        // 既に欠損していない場合
        if (SparseIncompleteData[tmprow].elementIndex(tmpcol) > 0 && SparseIncompleteData[tmprow].essencialSize() - c > 3) {
            // 要素を0にする
    ...
    for (int method = 0; method < (int)dir.size(); method++) {
        ...
        while (getline(ifs_mae, str)) {
            ...
            if (colIndex > std::stoi(tmp)) {
                std::cout << "precision_summary2: Structure of File does not correct.(" << method_name() << "MAE.txt)" << std::endl;
                return;
        ...
        while (getline(ifs_mae, str)) {
            ...
            if (colIndex > std::stoi(tmp)) {
                std::cout << "precision_summary2: Structure of File does not correct.(" << method_name() << "Fmeasure.txt)" << std::endl;
                return;
            } else if (colIndex < std::stoi(tmp)) {
        ...
        */
            ...
            for (int i = 0; i < P.rows(); i++) {
                for (int j = 0; j < SparseIncompleteData[i].essencialSize(); j++) {
                    if (SparseIncompleteData[i].elementIndex(j) != 0) {
                        ...
                        // std::cout << "err(" << i << "," << j  << "):" << err << "|" << P[i] * Q[SparseIncompleteData[i].indexIndex(j)] <<
                        // std::endl; 更新式で更新
                        for (int k_i = 0; k_i < K; k_i++) {
        ...
        if (ParameterNaN) {
            ...
            for (int index = 0; index < Missing; index++) {
                ...
                // std::cout <<"Prediction:"<<Prediction[index]<< " SparseCorrectData:" <<
                // SparseCorrectData[KessonIndex[index][0]].elementIndex(KessonIndex[index][1]) <<std::endl; std::cout << Prediction[index]-
                // SparseCorrectData[KessonIndex[index][0]].elementIndex(KessonIndex[index][1])<<std::endl;
            }
    ...
    for (int cluster_index = 0; cluster_index < Mem.rows(); cluster_index++) {
        ...
        for (int rand_mf_trial = 0; rand_mf_trial < trialLimit; rand_mf_trial++) {
            ...
            for (int step = 0; step < steps; step++) {
                ...
                double diff = frobenius_norm(prev_P - P) + frobenius_norm(prev_Q - Q);
#else
                double diff = fabs((error - prev_error) / error);
                prev_error =error;
#endif
                // diff出力
                //  std::ofstream ofs_diff(dir + "/diff/diff" + std::to_string(rand_mf_trial) + ".txt", std::ios::app);
...
int Recom::tfcmf_pred(std::string dir, double K_percent, int steps, int C, double Lambda, double FuzzifierEm, double beta,
                      double alpha) {  // はクラスタ数
    ...
    // trialLimit = 1; //debug
    for (int rand_mf_trial = 0; rand_mf_trial < trialLimit; rand_mf_trial++) {
        std::cout << "TFCMF: initial setting " << rand_mf_trial << std::endl;
        // P, Qの初期値を乱数で決定
                ...
                for (int j = 0; j < C; j++) {
                    ...
                    for (int k = 0; k < return_user_number(); k++) {
                        if ((Dissimilarities.Element[i].Element[k] != 0.0) || (Membership.Element[i].Element[k] != 0.0)) {
                            a_numerator += pow(Membership.Element[j].Element[k], FuzzifierEm) *
                                           (1 - Lambda * (1 - FuzzifierEm) * Dissimilarities.Element[j].Element[k]);
                            a_denominator += pow(Membership.Element[i].Element[k], FuzzifierEm) *
                                             (1 - Lambda * (1 - FuzzifierEm) * Dissimilarities.Element[i].Element[k]);
                        }
                    }
...
int Recom::nmf_pred(std::string dir, double K_percent,
                    int steps) {  // K以外は初期値有なので指定無しでも可 betaは熱田先輩のやつでいうγ　alphaは学習率　kは潜在次元数？ 　dirも謎
    int K;
    if (return_user_number() > return_item_number()) {
        K = std::round(return_item_number() * K_percent / 100);
    } else {
...
int Recom::wnmf_pred_after_clustering(std::string dir, double K_percent, int steps)  // K以外は初期値有なので指定無しでも可
{
    for (int cluster_index = 0; cluster_index < Mem.rows(); cluster_index++) {
        // クラスタに属するユーザ数と，それらが評価したアイテム数
        int cluster_user_num = 0, cluster_item_num = 0;
        ...
        if (steps < 50) {
            std::cerr << "QRFCM+WNMF : \"step\" should be 50 or more.";
            return 1;
        }
        std::cout << "QRFCM+WNMF || K:" << K << std::endl;
        ...
        // 計算のためmatrixにデータを変換
        SparseMatrix CulusterIncompleteData(cluster_user_num, return_item_number());
        int k = 0;
        for (int i = 0; i < SparseIncompleteData.rows(); i++) {
            if (Mem[cluster_index][i] == 1.0) {
                CulusterIncompleteData[k] = SparseIncompleteData[i];
                k++;
            }
        }

        Matrix Sij(cluster_user_num, return_item_number(), 0.0);

        // 欠損場所をSijに与える
        for (int i = 0; i < CulusterIncompleteData.rows(); i++) {
            for (int j = 0; j < CulusterIncompleteData[i].essencialSize(); j++) {
                if (CulusterIncompleteData[i].elementIndex(j) != 0) Sij[i][CulusterIncompleteData[i].indexIndex(j)] = 1.0;
            }
        }

        Matrix P(cluster_user_num, K), Q(return_item_number(), K);
        double best_error = DBL_MAX;
        int NaNcount = 0;
        int trialLimit = CLUSTERINGTRIALS;
        int mf_seed = 0;
        ...
        // trialLimit = 1; //debug
        for (int rand_mf_trial = 0; rand_mf_trial < trialLimit; rand_mf_trial++) {
            std::cout << "QRFCM+WNMF: initial setting " << rand_mf_trial << std::endl;
            // P, Qの初期値を乱数で決定
            std::mt19937_64 mt;
            for (int k_i = 0; k_i < K; k_i++) {
                for (int i = 0; i < P.rows(); i++) {
            ...
            SparseMatrix SMData = S_Hadamard(CulusterIncompleteData, Sij);
            SparseMatrix transpose_SMData = S_Hadamard(transpose(CulusterIncompleteData), transpose(Sij));
            ...
            for (int step = 0; step < steps; step++) {
                ...
                for (int i = 0; i < P.rows(); i++) {
                    for (int j = 0; j < CulusterIncompleteData[i].essencialSize(); j++) {
                        if (CulusterIncompleteData[i].elementIndex(j) != 0) {
                            err_num++;
                            error += pow(CulusterIncompleteData[i].elementIndex(j) - (P[i] * Q[CulusterIncompleteData[i].indexIndex(j)]), 2);
                        }
                    }
                }
#ifdef ARTIFICIALITY
...
int Recom::qfcnmf_pred(std::string dir, double K_percent, int steps, int C, double Lambda, double FuzzifierEm) {  // はクラスタ数
    ...
    if (steps < 50) {
        std::cerr << "QFCNMF_a: \"step\" should be 50 or more.";
        return 1;
    }
    ...
    // trialLimit = 1; //debug
    for (int rand_mf_trial = 0; rand_mf_trial < trialLimit; rand_mf_trial++) {
        std::cout << "QFCNMF_a: initial setting " << rand_mf_trial << std::endl;
        // P, Qの初期値を乱数で決定
        // 疑問:k次元の初期値の設定の仕方は同じでいいのか。
        std::mt19937_64 mt;
        for (int c = 0; c < C; c++) {
        ...
        for (int step = 0; step < steps; step++) {
            ...
            for (int i = 0; i < C; i++) {
                ...
                H_denominator = transpose(W[i]) * Matrix(tmp, "diag") * W[i] * H[i];
                ...
                for (int row = 0; row < H[i].rows(); row++) {
                    for (int col = 0; col < H[i].cols(); col++) {
                        if (H_numerator[row][col] != 0 && H_denominator[row][col] != 0 && H[i][row][col] != 0.0) {
                            ...
                            // std::cout <<"H_denominator[row][col]:"<<H_denominator[row][col] << " H_numerator[row][col]:"<< H_numerator[row][col]
                            // <<std::endl; std::cout <<"Data:"<<IncompleteData[row][col] << " H:"<< H[i][row][col] << " W:"<< W[i][row][col]
                            // <<std::endl;
                        }
                    }
                }
            ...
            //         double denominator=0.0;
            //         for(int j=0;j<C;j++){
            //           denominator+= pow(pow(Membership[j][k],FuzzifierEm)*(1.0-Lambda*(1.0-FuzzifierEm)*Dissimilarities[j][k])
            //                             /pow(Membership[i][k],FuzzifierEm)*(1.0-Lambda*(1.0-FuzzifierEm)*Dissimilarities[i][k]), 1.0/(FuzzifierEm));
            //         }
            ...
            if (diff < 0.011 && step >= 50) {
                ...
                for (int p = 0; p < K; p++) {
                    for (int i = 0; i < C; i++) {
                        for (int u = 0; u < 80; u++) {
                            for (int j = 0; j < 100; j++) {
                                Pre[u][j] += Membership[i][u] * W[i][u][p] * H[i][p][j];
                            }
                        }
                    }
                }
                ...
                for (int j = 0; j < return_user_number(); j++) {
                    error += pow(clusters_size[i], 1 - FuzzifierEm) * pow(Membership[i][j], FuzzifierEm) * Dissimilarities[i][j] +
                             1.0 / (Lambda * (FuzzifierEm - 1)) *
                                 (pow(clusters_size[i], 1 - FuzzifierEm) * pow(Membership[i][j], FuzzifierEm) - Membership[i][j]);
                }
            }
            std::cout << "#f:" << error << ":step:" << step << "\n";
            if (step == steps - 1) {
                std::cout << "step = " << step << ", error = " << error << ", K: " << K_percent << std::endl;
                ParameterNaN = true;
            }
...
int Recom::tfcnmf_pred(std::string dir, double K_percent, int steps, int C, double Lambda, double FuzzifierEm) {  // はクラスタ数
            ...
            for (int k = 0; k < C.height(); k++) {
                ...
                for (int i = 0; i < C.rows(); i++) {
                    for (int j = 0; j < 5; j++) {
                        ...
                        // std::cout << "MPBMF: Z " << k << ":" << i << ":" << SparseIncompleteData[i].indexIndex(j) << ":" << clock() << ":" <<
                        ...
                        if (SparseIncompleteData[i].elementIndex(j) != 0) {
                            ...
                            // std::cout << "err(" << i << "," << j  << "):" << err << "|" << U[k][i]*V[k][SparseIncompleteData[i].indexIndex(j)] <<
                            // std::endl; std::cout << "K-err(" << i << "," << j  << "):" << err << "|" <<
                            // w[k]*U[k][i]*V[k][SparseIncompleteData[i].indexIndex(j)] << std::endl;
                            /*
                            ...
                            for (int d = 0; d < D; d++) {
                                ...
                                */
                                ...
                                /*std::cout << sigma << "," << Z[k][i][SparseIncompleteData[i].indexIndex(j)] << " | "
                                   ...
                                   << " | " << V[k][SparseIncompleteData[i].indexIndex(j)][d] / sigma_v << std::endl;
                                */
                                ...
                                if (count_V[SparseIncompleteData[i].indexIndex(j)] == 0) {
                                    // V[k][SparseIncompleteData[i].indexIndex(j)][d] += alpha * (prev_V[k][SparseIncompleteData[i].indexIndex(j)][d]
                                    // / sigma_v );
                                    count_V[SparseIncompleteData[i].indexIndex(j)] = 1;
                                }
                                // std::cout << "U(" << i << "," << d << "):" << U[k][i][d] << "  V(" << j << "," << d << "):" <<
                                // V[k][SparseIncompleteData[i].indexIndex(j)][d] << "|" << Z[k][i][SparseIncompleteData[i].indexIndex(j)] <<
            ...
            for (int line = 0; line < X.rows(); line++) {
                ...
                if (Y[line] != 0) {
                    ...
                    for (int i = 0; i < X.cols(); i++) {
                        ...
                        for (int factor = 0; factor < K; factor++) {
                            V[factor][i] +=
                                alpha * (err * (X[line][i] * sum[factor] - V[factor][i] * X[line][i] * X[line][i]) - reg_v * V[factor][i]);
...
//                             for (int factor = 0; factor < K; factor++) {
//                                 V[c][factor][i] +=
//                                     alpha * (pow(Membership[c][i],FuzzifierEm)* err * (X[line].elementIndex(i) * sum[factor] - V[c][factor][i] *
//                                     X[line].elementIndex(i) * X[line].elementIndex(i)) - reg_v * V[c][factor][i]);
...
//                 for (int p = 0; p < K; p++) {
//                     for (int i = 0; i < C; i++) {
//                         Prediction[index] += Membership[i][KessonIndex[index][0]] * P[i][KessonIndex[index][0]][p] *
...
//         for(int j = 0; j < return_user_number(); j++){
//             error +=
//             pow(clusters_size[i],1-FuzzifierEm)*pow(Membership[i][j],FuzzifierEm)*Dissimilarities[i][j]+1/(Lambda*(FuzzifierEm-1))*(pow(clusters_size[i],FuzzifierEm)*pow(Membership[i][j],FuzzifierEm)-Membership[i][j]);
//           }
//           for(int k = 0; k < return_user_number(); k++){
//             P_L2Norm +=P[i][k] * P[i][k];
//           }
...
int Recom::fm_als_pred(std::string dir, double K_percent, double beta, double alpha, int steps)  // K以外は初期値有なので指定無しでも可
{
    ...
    for (int rand_mf_trial = 0; rand_mf_trial < trialLimit; rand_mf_trial++) {
        ...
        for (int step = 0; step < steps; step++) {
            ...
            double sum_e = 0;
            for (int i = 0; i < data_num; i++) sum_e += e[i];
            double w0a = -(sum_e - data_num * w0) / data_num;

            for (int l = 0; l < data_num; ++l) e[l] += (w0a - w0);
            w0 = w0a;
...
//                             tmp_X.Index[1] = return_user_number() + SparseIncompleteData.Element[k].Index[j];
//                             Dissimilarities.Element[c].Element[k] +=
//                                 (SparseIncompleteData.Element[k].Element[j] - fm_y_hat(tmp_X, w0[c], w[c], v[c]));
//                             error += Dissimilarities.Element[c].Element[k];
//                         }
//                     }
//                 }
...
//             double diff = diff_u + diff_v + diff_w + diff_w0;

//             std::cout << "step = " << step << ", error = " << error << ", diff: " << diff << ", diff_u: " << diff_u << ", diff_v: " << diff_v
//                       << ", diff_w: " << diff_w << ", diff_w0: " << diff_w0 << std::endl;
...
//                 pre_X[index].indexIndex(1) = KessonIndex[index][1] + return_user_number();
//                 for (int c = 0; c < C; c++) {
//                     Prediction[index] += fm_y_hat(pre_X[index], w0[c], w[c], v[c]) / C;
//                 }
//                 std::cout << "Prediction:" << Prediction[index];
//                 std::cout << " SparseCorrectData:" << SparseCorrectData[KessonIndex[index][0]].elementIndex(KessonIndex[index][1]);
//                 // std::cout<< Prediction[index]- SparseCorrectData[KessonIndex[index][0]].elementIndex(KessonIndex[index][1])
...
void Recom::Ubukata(int clusters_number) {
    for (int index = 0; index < Missing; index++) {
        ...
        for (int i = 0; i < clusters_number; i++) {
            ...
            // if(index==9973)
            // std::cout << Mem[i][KessonIndex[index][0]]<<"\t" << ItemMem[i][KessonIndex[index][1]]<<"\t"
            // <<Mem[i][KessonIndex[index][0]]*ItemMem[i][KessonIndex[index][1]]<< "\t" <<prediction[KessonIndex[index][1]]<< std::endl; std::cout
            // <<index<<"\t"<<SparseCorrectData[KessonIndex[index][0]].elementIndex(SparseIndex[index])<< "\t" <<Prediction[index]<< std::endl;
        }
        ...
        // <<Prediction[index]<< std::endl; std::cout << Prediction[index] << std::endl;
    }
    // std::cout << std::endl;
    return;
}
...
void Recom::UbukataForSphericalData(int clusters_number) {
    ...
    for (int index = 0; index < Missing; index++) {
        ...
        for (int i = 0; i < clusters_number; i++) {
            prediction += Mem[i][KessonIndex[index][0]] * ItemMem[i];
            // std::cout << Membership[i][KessonIndex[index][0]]<<"\t" << Centers[i][KessonIndex[index][1]]<<"\t" <<
            // Membership[i][KessonIndex[index][0]]*Centers[i][KessonIndex[index][1]]<< "\t" << Prediction[index]<< std::endl; std::cout
            // <<index<<"\t"<<SparseCorrectData[KessonIndex[index][0]].elementIndex(SparseIndex[index])<< "\t" <<Prediction[index]<< std::endl;
        }
        Prediction[index] = prediction[KessonIndex[index][1]] / squared_norm(prediction);
        // std::cout <<index<<"\t"<<
        // KessonIndex[index][0]<<":"<<KessonIndex[index][1]<<"\t"<<SparseCorrectData[KessonIndex[index][0]].elementIndex(SparseIndex[index])<< "\t"
        // <<Prediction[index]<< std::endl; std::cout << Prediction[index] << std::endl; std::cout
        // <<index<<"\t"<<SparseCorrectData[KessonIndex[index][0]].elementIndex(SparseIndex[index])<< "\t" <<Prediction[index]<< std::endl;
    }
    // std::cout << std::endl;
    return;
...
void Recom::restore_aveData(void) {
    ...
    for (int index = 0; index < Missing; index++) {
        ...
        // std::cout
        // <<index<<"\t"<<KessonIndex[index][0]<<":"<<KessonIndex[index][1]<<"\t"<<SparseCorrectData[KessonIndex[index][0]].elementIndex(SparseIndex[index])<<
        // "\t" <<Prediction[index]<<"\t"<<aveData[KessonIndex[index][0]]<< std::endl;
    }
    return;
}
